...
private karyawanData = {
  ...
  private channelName: string | null = 'JADIBOT BATAK VIP';
  ...
  private loadBotSettings() {
    try {
      ...
      if (obj.channelJid !== undefined) this.channelJid = obj.channelJid;
      if (obj.channelName !== undefined) this.channelName = obj.channelName;
...
private loadKaryawanData() {
  ...
  private async sendLoadingAnimation(jid: string, quoted: any) {
      const loadingFrames = [
          "████░░░░░░░░░░ 20%",
          "██████░░░░░░░░ 40%",
          "████████░░░░░░ 60%",
      ...
      try {
          let sentMsg = await this.sock.sendMessage(jid, { text: loadingFrames[0] }, { quoted });
          if (!sentMsg) return null;
          
          for (let i = 1; i < loadingFrames.length; i++) {
              await new Promise(resolve => setTimeout(resolve, 300));
              await this.sock.sendMessage(jid, { text: loadingFrames[i], edit: sentMsg.key });
          }
          await new Promise(resolve => setTimeout(resolve, 150));
          return sentMsg;
      } catch (e) {
          console.error("Error sending loading animation:", e);
          return null;
      }
  }
  ...
  private async handleIncomingMessage(msg: any) {
    ...
    if (possibleCommandName === "daftar") {
        ...
        if (parts.length < 2) {
            ...
            return await this.sock.sendMessage(jid, { text: errText, contextInfo }, { quoted: msg });
        }
        ...
        if (this.coverImageBuffer) {
            await this.sock.sendMessage(jid, { image: this.coverImageBuffer, caption: successText, contextInfo }, { quoted: msg });
        } else {
            await this.sock.sendMessage(jid, { text: successText, contextInfo }, { quoted: msg });
        }
        return;
    }
...
Ketik menu yang kamu inginkan.`;
      ...
      await this.sendMenuWithCover(jid, iqcText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, storeText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, beritaText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, sulapText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, tiketText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, karyawanText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
...
│ .infowarn - info peringatan saya/seseorang`;
      await this.sendMenuWithCover(jid, groupText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
...
│ .wallpaper - cari wallpaper keren`;
      ...
      await this.sendMenuWithCover(jid, kristenText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, cecanText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, animeText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, sertifikatText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      if (!text) {
         ...
         try {
             ...
             const cleanName = text.substring(0, 20);
             const TextToSVG = (await import('text-to-svg')).default;
             const fontPath = await this.getValidFontPath();
             const textToSVG = TextToSVG.loadSync(fontPath);
      ...
      await this.sendMenuWithCover(jid, rpgText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, primbonText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, margaText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, videoText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
      ...
      await this.sendMenuWithCover(jid, gameText, this.getFakeMenuQuote(senderJid, msg.pushName || "User"));
...
Pesan pribadi kembali diizinkan.` }, { quoted: msg });
    ...
    if(args) {
    ...
    } else if (body.startsWith(".addwalinkcha") || body.startsWith("addwalinkcha")) {
      if (!isOwner) return await this.sock.sendMessage(jid, { text: `👑 *Akses Ditolak*\nPerintah ini hanya bisa digunakan oleh Owner!` }, { quoted: msg });
      ...
      if (args.includes("whatsapp.com/channel/")) {
          const code = args.split("whatsapp.com/channel/")[1].split("/")[0].split("?")[0].trim();
          try {
              const meta = await this.sock.newsletterMetadata("invite", code);
              if (meta && meta.id) {
                  this.channelJid = meta.id;
                  this.channelName = meta.name || this.textNama || this.customBotName || 'JADIBOT BATAK VIP';
                  this.saveBotSettings();
                  return await this.sock.sendMessage(jid, { text: `✅ Berhasil mengatur link channel.\nNama: ${this.channelName}\nJID: ${this.channelJid}` }, { quoted: msg });
              } else {
                  return await this.sock.sendMessage(jid, { text: "❌ Gagal mendapatkan data channel dari link tersebut. Pastikan link valid." }, { quoted: msg });
              }
          } catch(e: any) {
              return await this.sock.sendMessage(jid, { text: `❌ Gagal memproses link channel. Pastikan link benar dan bot tidak diblokir. Error: ${e.message || e}` }, { quoted: msg });
          }
      } else {
          // Asumsi user memasukkan JID secara manual
          if (!newJid.endsWith("@newsletter")) newJid = newJid + "@newsletter";
          this.channelJid = newJid;
          this.saveBotSettings();
          await this.sock.sendMessage(jid, { text: `✅ Berhasil mengatur JID channel menjadi: ${newJid}` }, { quoted: msg });
      }
    } else if (body.startsWith(".delwalinkcha") || body.startsWith("delwalinkcha")) {
      if (!isOwner) return await this.sock.sendMessage(jid, { text: `👑 *Akses Ditolak*\nPerintah ini hanya bisa digunakan oleh Owner!` }, { quoted: msg });
      this.channelJid = null;
...
🆔 LID: ${lid}` }, { quoted: msg });
      ...
      if (targetQuery === "videosexybikini") {
        const hotQueries = ["tiktok bikini compilation", "tiktok swimsuit compilation", "tiktok bikini haul", "tiktok bikini model"];
        searchQuery = hotQueries[Math.floor(Math.random() * hotQueries.length)];
      } else if (targetQuery === "vidsexyjepang") {
        const hotQueries = ["tiktok bikini japan", "tiktok japanese swimsuit", "tiktok japan bikini compilation"];
        searchQuery = hotQueries[Math.floor(Math.random() * hotQueries.length)];
      } else if (targetQuery === "vidsexyindonesia") {
        const hotQueries = ["tiktok bikini indonesia", "tiktok indo bikini", "tiktok bikini lokal indonesia"];
        searchQuery = hotQueries[Math.floor(Math.random() * hotQueries.length)];
      } else if (targetQuery === "vidsexymalaysia") {
        const hotQueries = ["tiktok bikini malaysia", "tiktok amoi malaysia bikini", "tiktok malaysia swimsuit"];
        searchQuery = hotQueries[Math.floor(Math.random() * hotQueries.length)];
      } else if (targetQuery === "vidsexychina") {
        const hotQueries = ["tiktok bikini china", "tiktok chinese bikini", "douyin tiktok bikini"];
        searchQuery = hotQueries[Math.floor(Math.random() * hotQueries.length)];
           ...
           try {
              ...
              if (fallbackSearch && fallbackSearch.videos && fallbackSearch.videos.length > 0) {
                 ...
                 videos = fallbackSearch.videos.map(v => ({
                    ...
                    title: v.title,
                    type: 'video',
                    seconds: v.duration ? ((v.duration as any).seconds || 0) : 0
...
│ .losmenmelati`;
      ...
      if (body.includes("on")) {
       ...
       try {
           ...
           let svgPaths = '';
           const fontOptions = { fontSize: 60, anchor: 'center middle', attributes: { fill: 'black' } };
           
           const startY = 256 - ((lines.length - 1) * 35);
           
           for (let i = 0; i < lines.length; i++) {
               svgPaths += textToSVG.getPath(lines[i].trim(), { ...fontOptions, x: 256, y: startY + (i * 70) });
           }
           ...
           await this.sock.sendMessage(jid, { text: `❌ Gagal membuat stiker brat: ${e.message || e}` }, { quoted: msg });
       }
       ...
       if (!text || !text.includes("|")) {
          await this.sock.sendMessage(jid, { text: `Kirim teks dengan format atas|bawah!\nContoh: .smeme Halo|Semua` }, { quoted: msg });
           ...
           const isQuotedImage = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.imageMessage;
           const isImage = msg.message?.imageMessage;
           const messageToDownload = isQuotedImage ? { message: { imageMessage: isQuotedImage } } : isImage ? msg : null;
           
           if (!messageToDownload) {
               await this.sock.sendMessage(jid, { text: `Reply/kirim gambar dengan caption .smeme atas|bawah` }, { quoted: msg });
               return;
           }
           
           try {
             await this.sock.sendMessage(jid, { text: "⏳ *Membuat stiker meme...*" }, { quoted: msg });
             const stream = await downloadContentFromMessage(messageToDownload.message.imageMessage, 'image');
             let buffer = Buffer.from([]);
             for await(const chunk of stream) buffer = Buffer.concat([buffer, chunk]);
             
             const bgBuffer = await sharp(buffer).resize(512, 512, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } }).toBuffer();
             
             const TextToSVG = (await import('text-to-svg')).default;
             const fontPath = await this.getValidFontPath();
           const textToSVG = TextToSVG.loadSync(fontPath);
             
             const fontOptions = { fontSize: 45, anchor: 'center middle', attributes: { fill: 'white', stroke: 'black', 'stroke-width': 2 } };
             
             let svgPaths = '';
             svgPaths += textToSVG.getPath(atas.trim(), { ...fontOptions, x: 256, y: 50 });
             svgPaths += textToSVG.getPath(bawah.trim(), { ...fontOptions, x: 256, y: 462 });
             
             const svg = `<svg width="512" height="512" xmlns="http://www.w3.org/2000/svg">${svgPaths}</svg>`;
             ...
             await this.sock.sendMessage(jid, { sticker: stickerBuffer }, { quoted: msg });
               ...
               await this.sock.sendMessage(jid, { text: `❌ Gagal membuat stiker meme: ${e.message || e}` }, { quoted: msg });
           }
       }
       ...
       const text = messageContent.replace(/^\.?iqc(pink|merah|biru|kuning)?\s*/i, "").trim();
       const cmdName = body.split(" ")[0].replace(".", "").toLowerCase();
       if (!text) {
          await this.sock.sendMessage(jid, { text: `Kirim teks untuk dibuat IQC!\nContoh: .iqcpink Halo semuanya` }, { quoted: msg });
       } else {
          await this.sock.sendMessage(jid, { text: `⏳ Sedang membuat IQC...` }, { quoted: msg });
          try {
             const iqcPkg = require('iqc-canvas');
             const generateIQC = iqcPkg.generateIQC || iqcPkg.default?.generateIQC;
             
             let bubbleColor = '#1a1a1a';
             let textColor = '#ffffff';
             
             if (cmdName === 'iqcpink') { bubbleColor = '#ff69b4'; textColor = '#ffffff'; }
             else if (cmdName === 'iqcmerah') { bubbleColor = '#ff3b30'; textColor = '#ffffff'; }
             else if (cmdName === 'iqcbiru') { bubbleColor = '#007aff'; textColor = '#ffffff'; }
             else if (cmdName === 'iqckuning') { bubbleColor = '#ffcc00'; textColor = '#000000'; }
             
             const date = new Date();
             const time = `${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`;
             
             // Ensure the user's avatar is fetched if possible, though iqc-canvas doesn't natively take avatar unless we pass it, wait...
             // iqc-canvas actually fetches avatar automatically or uses default in the library if we don't supply one?
             // No, iqc-canvas doesn't have avatar! It's just text and reactions! Wait, iOS context menus don't have avatars, they are just the bubble!
             const result = await generateIQC(text, time, { bubbleColor, textColor });
             
             if (result && result.image) {
                await this.sock.sendMessage(jid, { image: result.image, caption: 'IQC by AI Studio' }, { quoted: msg });
             } else {
                throw new Error("Invalid result from iqc-canvas");
             }
          } catch (e) {
             console.error("IQC error: ", e);
             await this.sock.sendMessage(jid, { text: `❌ Gagal membuat IQC: ${e.message || e}` }, { quoted: msg });
          }
       }
       ...
       if (messageToDownload) {
           try {
               ...
               const imageBuffer = await sharp(buffer).resize(512, 512, { fit: 'cover' }).toBuffer();
           ...
           const textToSVG = TextToSVG.loadSync(fontPath);
               ...
               let svgPaths = '';
               const fontOptions = { fontSize: 60, anchor: 'center middle', attributes: { fill: 'white', stroke: 'black', 'stroke-width': 2 } };
               ...
               await this.sock.sendMessage(jid, { text: `❌ Gagal membuat stiker brat gambar: ${e.message || e}` }, { quoted: msg });
           }
       } else {
      ...
      await this.sock.sendMessage(jid, { text: `📊 *${cmdName.toUpperCase()}*\n\nTingkat ${cmdName.replace("cek", "")} kamu adalah: *${percentage}%*` }, { quoted: msg });
       ...
       if (!teks) {
         ...
         try {
           ...
           let svgPaths = '';
           const fontOptions20 = { fontSize: 20, anchor: 'top baseline', attributes: { fill: '#1b1b1b' } };
           const fontOptions18 = { fontSize: 18, anchor: 'top baseline', attributes: { fill: '#1b1b1b' } };
           
           svgPaths += textToSVG.getPath(hari, { ...fontOptions20, x: 806, y: 78 });
           svgPaths += textToSVG.getPath(tanggal, { ...fontOptions18, x: 806, y: 102 });
           svgPaths += textToSVG.getPath(safeName, { ...fontOptions18, x: 360, y: 100 });
           svgPaths += textToSVG.getPath('-', { ...fontOptions18, x: 360, y: 120 });
           
           for (let i = 0; i < lines.length; i++) {
               svgPaths += textToSVG.getPath(lines[i].trim(), { ...fontOptions20, x: 344, y: 142 + (i * 19) });
           }
           
           const svgText = `<svg width="1024" height="784" xmlns="http://www.w3.org/2000/svg">${svgPaths}</svg>`;
           ...
           await this.sock.sendMessage(jid, { text: `❌ *Gagal menulis:* ${e.message || e}` }, { quoted: msg });
         }
       }
...
*Sumber*: BMKG`;
      ...
      if (isAvatar) {
        ...
        try {
          ...
          await this.sock.sendMessage(jid, { image: { url: randomCouple.male }, caption: "Cowok" }, { quoted: msg });
          await this.sock.sendMessage(jid, { image: { url: randomCouple.female }, caption: "Cewek" }, { quoted: msg });
        } catch (e) {
          console.error("Error fetching ppcouple:", e);
          await this.sock.sendMessage(jid, { text: "Gagal mengambil ppcouple. Coba lagi nanti." }, { quoted: msg });
        }
      }
       ...
       if (!text) {
          ...
          try {
           ...
           const textToSVG = TextToSVG.loadSync(fontPath);
             ...
             let svgPaths = '';
             const fontOptions = { fontSize: 80, anchor: 'center middle', attributes: { fill: randomColor, stroke: 'white', 'stroke-width': 4 } };
             
             const startY = 256 - ((lines.length - 1) * 45);
             for (let i = 0; i < lines.length; i++) {
                 svgPaths += textToSVG.getPath(lines[i].trim(), { ...fontOptions, x: 256, y: startY + (i * 90) });
             }
             ...
             await this.sock.sendMessage(jid, { text: `❌ Gagal membuat ATTP: ${e.message || e}` }, { quoted: msg });
          }
       }
       ...
       if (!text) {
          ...
          try {
           ...
           const textToSVG = TextToSVG.loadSync(fontPath);
             ...
             await this.sock.sendMessage(jid, { image: imageBuffer, caption: `🎨 *Logo Selesai*` }, { quoted: msg });
          } catch (e) {
             console.error("Logo error: ", e);
             await this.sock.sendMessage(jid, { text: `❌ Gagal membuat logo: ${e.message || e}` }, { quoted: msg });
          }
       }
